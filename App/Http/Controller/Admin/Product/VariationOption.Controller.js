...
class VariationOptionController extends Controller{
  async createVariationOption(req, res, next){
    try {
      ...
      const chechId = await checkExistOfModelById(variationId, VariationModel);
      const createResault = await VariationOptionModel.create({variation_Id: chechId._id, title});
      if(!createResault) throw new createHttpError.InternalServerError("خطای سروری");
      return res.status(httpStatus.CREATED).json({
        statusCode: httpStatus.CREATED,
        data: {
          message: "مقادیر جزئیات محصول با موفقیت ثبت شد"
        }
      });
    } catch (error) {
      next(error)
  ...
  async listOfVariationOptionByVariationId(req, res, next){
    try {
      const { id } = req.params;
      const checkId = await checkExistOfModelById(id, VariationModel);
      const variationOption = await VariationOptionModel.find({variation_Id: checkId._id}, {__v: 0, variation_Id: 0});
      const numberOfresault = await VariationOptionModel.find({variation_Id: checkId._id}).count();
      if(!variationOption) throw new createHttpError.NotFound("گزینه ی مورد نظر یافت نشد");
